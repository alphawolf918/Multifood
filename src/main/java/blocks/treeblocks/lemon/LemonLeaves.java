package multifood.blocks.treeblocks.lemon;
...
public class LemonLeaves extends BlockLeaves implements IShearable
{
	public static final String[] LEAF_TYPES = new String[] { "lemon" };
	...
	public LemonLeaves(int par1)
	{
		super(par1);
		this.setTickRandomly(true);
		this.setCreativeTab(ModTabs.mfTab);
		this.setUnlocalizedName("lemonleaves");
			...
			int par4, int par5, float par6, int par7)
	{
		if (!par1World.isRemote)
		{
			...
			if (par1World.rand.nextInt(j1) <= 10)
			{
				int k1 = Blocks.lemonSapling.blockID;
			...
			if ((par5 & 3) == 0 && par1World.rand.nextInt(j1) <= 30)
			{
				this.dropBlockAsItem_do(par1World, par2, par3, par4,
						new ItemStack(Items.lemon, 1, 0));
			}
		}
	}
	...
	{
		this.blockIcon = par1IconRegister.registerIcon("awmf:lemonleaves");
	}
